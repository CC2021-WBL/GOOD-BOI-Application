  ...
  const changeInitialData = () => {
    ...
    try {
      if (chosenDog !== undefined && chosenDog !== null) {
        ...
        for (
          let index = 1;
          index < Object.keys(dogFormInitialState).length;
          index++
        ) {
          const element = Object.keys(dogFormInitialState)[index];
          console.log(element);
          setInitialStateOfDogForm((prevState) => {
            prevState[element] = dogFromDB[element] || 'brak danych';
            console.log(prevState[element]);
          });
          //modifiedInitialState[element] = dogFromDB[element];
        }
